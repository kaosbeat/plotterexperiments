...
def connectthedots(object1,object2):
	...
	plotter.write(g)
	kartelconnect((x1,y1),(x2,y2),abs(int(math.floor((x2-y2)/100))),object1.get('size'),object2.get('size'),True)
	# plotter.write(g)
...
def kartelconnect(p1,p2,size,size1,size2,log):
	...
	partsspace = np.logspace(0.7 ,2, parts, endpoint=True)
	...
	if (log == True):
		...
		g.append((partsspace[i+1]*xlength/100,partsYspace[i+1]),p2)	
...
def plot(start, end): #(left 0; bottom 0; right 16158; top 11040)
	...
	for x in xrange(1,3):
		addobject2(random.randint(0,8000),random.randint(0,5000),random.randint(220,2000))
		print(len(objects))
