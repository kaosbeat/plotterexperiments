	...
	for idx, wav in enumerate(wavperiod):
		...
		for i in range(0,rez):
			...
			if waveshape[idx] == "spiral":
				atom = spiral_archimedean(size*20, num_turns=22, wrapping_constant=10, direction='cw', segments=30)			
			if wavtype[idx] == "sin":
...
somewaves(True, False, [wav1, wav2, wav3], ["sin", "cos", "cos"], [2100, 3000, 1500], ["spiral", "cross", "circle"])
