...
def somewaves(waves, subwaves, wavperiod = [], wavtype = [], wavesize = [], waveshape = []):
	for idx, wav in enumerate(wavperiod):
		...
		if subwaves:
			p1 = getrandompoint(wav, wavtype[idx], 1500)
			p2 = getrandompoint(wav, wavtype[wavetype.length - 1], 1500)
			# print p1
...
somewaves(True, False, [wav1, wav2, wav3], ["sin", "cos", "cos"], [2100, 3000, 1500], ["rect", "cross", "circle"])
